...
const App: React.FC = () => {
  ...
  const handleGenerateLogo = useCallback(async () => {
    ...
    try {
      ...
      if (sourceLanguage !== 'English') {
        setLoadingMessage('Translating your description...');
        finalPrompt = await translateText(prompt, sourceLanguage);
      }
      
      setLoadingMessage('Conjuring up your masterpiece...');
      const imageB64 = await generateLogoImage(finalPrompt);
  ...
  const addStyleToPrompt = (style: string) => {
    setPrompt(prev => {
        // Remove existing style-related phrases before adding a new one
        const cleanedPrompt = prev.replace(/, [a-zA-Z]+ style\.$/i, '');
        // Add the new style
            ...
            <div className="lg:w-1/2 flex flex-col space-y-6">
              ...
              >
                {isLoading ? (
                  <>
                    <Spinner />
                    {loadingMessage.replace('Conjuring up your masterpiece...', 'Generating...')}
                  </>
                ) : (
                  '✨ Generate Super Logo'
                )}
